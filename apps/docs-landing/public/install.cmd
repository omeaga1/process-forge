...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$candidates = @('https://github.com/omeaga1/process-forge/releases/latest/download/process-forge-windows-portable-x64.zip','https://github.com/omeaga1/process-forge/releases/download/v0.1.2/process-forge-windows-portable-x64.zip','https://github.com/omeaga1/process-forge/releases/download/v0.1.1/process-forge-windows-portable-x64.zip','https://omeaga1.github.io/process-forge/process-forge-windows-portable-x64.zip','https://raw.githubusercontent.com/omeaga1/process-forge/main/release-dist/process-forge-windows-portable-x64.zip'); foreach ($u in $candidates) { try { Invoke-WebRequest -Uri $u -OutFile '%TEMP_ZIP%' -UseBasicParsing -TimeoutSec 30; if ((Test-Path '%TEMP_ZIP%') -and ((Get-Item '%TEMP_ZIP%').Length -gt 10000)) { break } } catch {} }"
